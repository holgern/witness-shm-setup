...
echo -e "${cyan}Enabling 6GB swap...${normal}"
dd if=/dev/zero of=/swapfile bs=1M count=6252
mkswap /swapfile
echo "/swapfile swap swap defaults 0 0" | sudo tee /etc/fstab

echo -e "${cyan}Set shared-file-size to 2*RAM + 6 -1 GB in config.ini...${normal}"
echo -e "${cyan}Assuming 37 GB ram now...${normal}"
